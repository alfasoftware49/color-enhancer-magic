// @ts-nocheck
/**
 * Partner role workspace — the source repo's /dashboard/$role screen, rendered
 * as a component so the Control Panel sidebar can open it directly.
 * Layout/behaviour copied as-is; only the router search params are replaced
 * with local state.
 */
...
export function RoleWorkspace({ role: initialRole }: { role: RoleKey }) {
  const [role, setRole] = useState<RoleKey>(initialRole);
  const [activeModule, setActiveModule] = useState<string | null>(null);
  const [kpiTone, setKpiTone] = useState<KpiTone>("all" as KpiTone);
  ...
  const availableTones = useMemo<KpiTone[]>(() => {
    ...
    return ["all", ...Array.from(set)] as KpiTone[];
  }, [cfg.kpis]);
          ...
          {isAIChat ? (
            ...
            <>
              <ResellerProfileHero
                ...
                bannerGradient={ROLE_BANNER_GRADIENTS[role]}
              />
              {role === "reseller" ? (
                <ResellerHero />
              ) : role === "vendor" ? (
                ...
                <Hero
                  role={cfg}
                  onCta={() => setActiveModule(cfg.modules[0]?.key ?? null)}
                  onAnalytics={() => setActiveModule(cfg.modules.find((m) => /analytic|report|insight/i.test(m.label))?.key ?? cfg.modules[0]?.key ?? null)}
                />
              )}
              <KpiToolbar
